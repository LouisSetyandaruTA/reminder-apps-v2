...
try:
    ...
    rename_mapping = {
        ...
        'No. Telp': 'phone', 'Nomor Telpon': 'phone'
    }
    ...
    df_long['serviceDate'] = pd.to_datetime(df_long['serviceDate'])

    # 4. TENTUKAN TANGGAL PEMBELIAN & TANGGAL SERVIS TERAKHIR
    # Tanggal pembelian/pemasangan
    if 'Pemasangan' in df.columns:
        df['purchaseDate'] = pd.to_datetime(df['Pemasangan'].apply(parse_date), errors='coerce')
    else:
        df['purchaseDate'] = None
    
    # Cari tanggal servis paling awal dan paling akhir untuk setiap pelanggan
    date_summary = df_long.groupby('name')['serviceDate'].agg(['min', 'max']).reset_index()
    date_summary.rename(columns={'min': 'earliest_service', 'max': 'latest_service'}, inplace=True)

    # Gabungkan summary tanggal ke data pelanggan
    customers_df = df.drop(columns=service_cols).drop_duplicates(subset=['name'])
    customers_df = pd.merge(customers_df, date_summary, on='name', how='left')

    # Tentukan purchaseDate final: prioritaskan Pemasangan, lalu servis paling awal
    ...
    # Format tanggal ke string untuk CSV
    ...
    # 5. SIAPKAN DATA AKHIR UNTUK DISIMPAN
