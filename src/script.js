document.addEventListener('DOMContentLoaded', () => {
    ...
    // In a real browser environment, you would fetch this data from a server.
    // This mock simulates the behavior of your Electron backend.
    const getMockData = () => ([
    ...
    // Simulate the Electron API bridge
    const electronAPI = {
        refreshData: async () => {
            ...
            // For this demo, we reset to mock data.
            customers = getMockData();
            return { success: true, data: customers };
        },
        updateService: async ({ rowIndex, serviceColumn, newDate }) => {
            console.log('API: Updating service', { rowIndex, serviceColumn, newDate });
        ...
        updateContactStatus: async ({ rowIndex, status, contactDate, notes }) => {
            console.log('API: Updating contact', { rowIndex, status, notes });
            const customer = customers.find(c => c.rowIndex === rowIndex);
            if (customer) {
                customer.contactStatus = status;
                customer.contactNotes = notes;
                // You might also want to store contactDate
                return { success: true };
            }
            return { success: false, error: 'Customer not found.' };
        },
        ...
        openWhatsApp: (phoneNumber) => {
            console.log('API: Opening WhatsApp for', phoneNumber);
    ...
    const renderCustomers = () => {
        ...
        sortedAndFilteredCustomers.forEach(customer => {
            ...
            const servicesHistoryHtml = customer.services && Object.keys(customer.services).length > 0
                ? Object.entries(customer.services)
                    .map(([key, value]) => `<div class="mt-2 text-xs bg-gray-50 p-2 rounded"><span class="text-gray-500">${key}:</span> ${formatDate(value)}</div>`)
                    .join('')
                : '<div class="mt-2 text-xs text-gray-500">No service history.</div>';
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <div class="flex items-center mb-2 flex-wrap">
                            <h3 class="text-lg font-semibold text-gray-900 mr-3">${customer.name}</h3>
                            <span class="px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(priority)} mr-2">
                                ${priority} Priority
                            </span>
                            <span class="px-2 py-1 rounded-full text-xs font-medium border ${contactStatusDisplay.color} flex items-center">
                                <i data-lucide="${contactStatusDisplay.icon}" class="w-3.5 h-3.5"></i>
                                <span class="ml-1">${contactStatusDisplay.text}</span>
                            </span>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div>
                                <p class="text-sm text-gray-600">Next Service Reminder</p>
                                <p class="font-medium">${formatDate(customer.nextService)}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Service Status</p>
                                <p class="font-medium text-blue-600">${serviceDays}</p>
                            </div>
                             <div>
                                <p class="text-sm text-gray-600">Last Service</p>
                                <p class="font-medium">${mostRecentService ? formatDate(mostRecentService) : 'N/A'}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Last Contact Notes</p>
                                <p class="font-medium italic text-gray-700">${customer.contactNotes || 'No notes'}</p>
                            </div>
                        </div>

                        <div class="mb-3">
                            <details class="group">
                                <summary class="text-sm font-medium text-gray-700 cursor-pointer hover:text-gray-900 list-none">
                    ...
                    <div class="ml-4 flex flex-col gap-2 items-end" style="min-width: 180px;">
                        <button data-action="call" data-phone="${customer.phone}" class="w-full px-3 py-2 text-xs bg-green-600 text-white rounded hover:bg-green-700 flex items-center justify-center whitespace-nowrap">
                            <i data-lucide="message-circle" class="w-3.5 h-3.5 mr-1"></i> Contact via Whatsapp
                        </button>
                        <button data-action="update-contact" data-row-index="${customer.rowIndex}" class="w-full px-3 py-2 text-xs bg-purple-600 text-white rounded hover:bg-purple-700 flex items-center justify-center whitespace-nowrap">
                            <i data-lucide="user-check" class="w-3.5 h-3.5 mr-1"></i> Update Contact Status
                        </button>
                        <button data-action="update-service" data-row-index="${customer.rowIndex}" class="w-full px-3 py-2 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center justify-center whitespace-nowrap">
                            <i data-lucide="settings" class="w-3.5 h-3.5 mr-1"></i> Update Services
                        </button>
                    </div>
                </div>`;
    ...
    const setupAndOpenServiceModal = (customer) => {
    ...
    const setupAndOpenContactModal = (customer) => {
        selectedCustomer = customer;
    ...
    const setupAndOpenAddCustomerModal = () => {
    ...
    customerListContainer.addEventListener('click', (e) => {
        ...
        if (action === 'call') {
            electronAPI.openWhatsApp(button.dataset.phone);
    ...
    document.getElementById('contact-modal-save').addEventListener('click', async () => {
        ...
        const result = await electronAPI.updateContactStatus({
            ...
            status,
            contactDate: status === 'contacted' ? todayStr : null,
            notes
        });
         if (result.success) {
    ...
    async function initializeApp() {
        ...
        try {
            ...
            lucide.createIcons(); // Final icon check
        }
    };
